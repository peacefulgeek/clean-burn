/**
 * Image pipeline — fetch, convert to WebP, compress to <200KB, upload to Bunny CDN.
...
export async function processAndUploadImage(sourceUrl, filename) {
  // Dynamic import sharp so it doesn't crash if not installed
  ...
  const uploadUrl = `https://storage.bunnycdn.com/${BUNNY_STORAGE_ZONE}/${finalName}`;
